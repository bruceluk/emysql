...
run() ->
	...
	log4erl:debug("~nCreate Record~n-------------~n", []),
	emysql:execute(hello_pool,
		<<"INSERT INTO hello_table SET hello_text = 'Hello World!'">>),

	log4erl:debug("~nTest Select~n-----------~n", []), 
	Result1 = emysql:execute(hello_pool,
  	<<"select * from hello_table limit 2">>),
	log4erl:debug("~p~n", [Result1]),

	log4erl:debug("~nCreate Stored Procedure~n-----------------------~n", []),
	Result2 = emysql:execute(hello_pool,
  	<<"create procedure sp_hello() begin select * from hello_table limit 2; end">>),
	log4erl:debug("~p~n", [Result2]),

	log4erl:debug("~nCall Stored Procedure~n---------------------~n", []),
	Result3 = emysql:execute(hello_pool,
	   	<<"call sp_hello();">>),
		log4erl:debug("~p~n", [Result3]),
